###################################################
#                                                 #
# This example script combines the two previous   #
# ones: it uses a logarithmically-spaced pixel    #
# grid, which spans several orders of magnitude   #
# in spatial scale, to make a full-Stokes         #
# (polarized) image, regrids the innermost part   #
# onto a regular grid, and plots the total        #
# intensity with polarization ticks along with    #
# the fractional linear and circular              #
# polarization.                                   #
#                                                 #
###################################################
...
print("integrated flux density over the full log-spaced image (Jy):")
...
###################################################
# regrid the innermost 200 rg onto a regular grid

# specify new grid
x_new = np.linspace(-100.0, 100.0, 200)
y_new = np.linspace(-100.0, 100.0, 200)

# interpolate each Stokes parameter -- reminder: interpolate intensity and not flux!
IQUV_new = np.stack(
    [
        jf.interp_to_regular_grid(S, x, y, x_new=x_new, y_new=y_new, method="linear")[2]
        for S in IQUV
    ]
)

# per-pixel maps; pixels below the floor are left as NaN so they do not colour the plot
floor = IQUV_new[0].max() * 1.0e-3
m_lin, m_circ, chi = jf.polarization_fractions(IQUV_new, floor=floor)

###################################################
# plot

Tb = jf.convert_units(model, IQUV_new[0], x_new, y_new, output_units="Tb", frequency=frequency)
...
pc = ax.pcolormesh(x_new, y_new, np.log10(Tb), cmap="afmhot", vmin=vmax - 3.0, vmax=vmax)
...
step = max(1, len(x_new) // 25)
X, Y = np.meshgrid(x_new[::step], y_new[::step])
ml = m_lin[::step, ::step]
ch = chi[::step, ::step]
scale = 0.9 * step * (x_new[1] - x_new[0]) / max(np.nanmax(ml), 1e-12)
...
ax.set_facecolor("black")
ax.set_aspect("equal")
pc = ax.pcolormesh(x_new, y_new, 100.0 * m_lin, cmap="viridis", vmin=0.0)
...
lim = 2.0
pc = ax.pcolormesh(x_new, y_new, 100.0 * m_circ, cmap="RdBu_r", vmin=-lim, vmax=lim)
...
plt.tight_layout()
plt.savefig("jet_image_log_polarized.png", dpi=200, bbox_inches="tight")
plt.close()
...
# first, convert the regridded image to flux density
Snu_new = np.stack(
    [jf.convert_units(model, S, x_new, y_new, output_units="flux", D=D) * 1.0e23 for S in IQUV_new]
)

# angular offsets, in radians
rad_per_rg = model.rg / (D * 3.086e24)
jf.export_fits(
    "jet_image_log_polarized.fits",
    Snu_new,
    x_new * rad_per_rg,
    y_new * rad_per_rg,
