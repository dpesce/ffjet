...
class JetModel:
    ...
    def _load_synchrotron_tables(self):
        ...
        # the tables exist on a 0.01-wide grid in p and the file name is generated by
        # rounding, so silently substituting a neighbouring table would be easy to miss
        if abs(p - round(p, 2)) > 1.0e-9:
            raise ValueError(
                f"p={p} is not on the tabulated grid (spacing 0.01); "
                f"use p={round(p, 2):.2f} or generate the tables for this p."
            )
        if not (2.0 < p <= 8.0):
            raise ValueError(
                f"p={p} is outside the usable range 2 < p <= 8 (tables are needed for "
                f"both p and p+1, and the electron normalization diverges as p -> 2)."
            )
        ...
        # log-space copies of the tables; the integrands fall exponentially at large x,
        # so interpolation must be done in log(x)-log(G), not linearly in x
        for _key in ("GI_2", "GI_p", "GI_pp1", "GaI_2", "GaI_p", "GaI_pp1"):
            _x = getattr(self, "x_" + _key)
            _G = getattr(self, _key)
            setattr(self, "logx_" + _key, np.log10(_x))
            setattr(self, "logG_" + _key, np.log10(np.maximum(_G, 1.0e-300)))
    ...
    # the left and zero returned to the right of the tabulated range
    def _G_lookup(self, x, key):
        logx = getattr(self, "logx_" + key)
        logG = getattr(self, "logG_" + key)
        L = np.log10(np.maximum(x, 1.0e-300))
        return 10.0 ** np.interp(L, logx, logG, left=logG[0], right=-np.inf)
